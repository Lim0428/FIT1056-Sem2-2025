...
class PatientService:
    ...
    def update_profile(self, patient_id: str, patch: dict):
        db = read_db()
    ...
    # Surveys
    def add_survey(self, patient_id: str, survey: dict):
        db = read_db()
        db["surveys"].setdefault(patient_id, [])
        db["surveys"][patient_id].append(survey)
        write_db(db)

    def get_surveys(self, patient_id: str):
        db = read_db()
        return db["surveys"].get(patient_id, [])

    # Feedback
    def add_feedback(self, patient_id: str, feedback: dict):
        db = read_db()
        db["feedback"].setdefault(patient_id, [])
        db["feedback"][patient_id].append(feedback)
        write_db(db)

    def get_feedback(self, patient_id: str):
        db = read_db()
