...
def page_dashboard(store):
    ...
    with left:
        with st.container(border=True):
            ...
            # Dark blue background, light blue bars, white text
            revenue_chart(
                years,
                values,
                x_label="Revenue (RM)",
    ...
    with mid:
        with st.container(border=True):
            ...
            today_appointments_list(store)  # <-- uses only JSON, rows are clickable

    with right:
        profile_card(store)  # doctor name + rating/Patients
    ...
    with m2:
        with st.container(border=True):
            ...
            appointment_history_list(store)  # <-- pass store (fixes your error)
